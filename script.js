...
function get_req() {
  ...
  get_request(req_url, make_filterv, '.results-container', '.loader-search');
}
      ...
      let div = `<div class="col-12 col-md-4 col-lg-3">\
                      <div class="card mcard">\
                        <div class="mcard-video">\
                          <img class="card-img-top mcard-video" src="${person.thumb_url}" alt="Card image cap">\
                          <img class="mcard-play" src=".images/play.png" alt="play">\
                        </div>\
                        <div class="card-body mcard">\
                          <h4 class="card-title mcard-title">${person.title}</h4>\
                          <p class="card-text mcard-text">${person["sub-title"]}</p>\
                          <div class="mcard-ins d-flex flex-row align-items-center mt-3 mb-2">\
                            <img class="mcard-img" src="${person.author_pic_url}" alt="Person smiling">\
                            <p class="mcard-name m-0 ml-2">${person.author}</p>\
                          </div>\
                          <div class="mcard-eval d-flex flex-row justify-content-between">\
                            <div class="mcard-constarts">`;

      let onstarts = person.start;
      for (let i = 0; i < onstarts; ++i) {
          div += '<img class="mcard-starts" src=".images/star_on.png" alt="Starts"></img>';
      }

      let offstarts = 5 - onstarts;

      for (let i = 0; i < offstarts; ++i) {
          div += '<img class="mcard-starts" src=".images/star_off.png" alt="Starts">';
      }
      div += `</div><p class="card-text mcard-time">${person.duration}</p></div></div></div></div>`;
...
function make_popvideos(data) {
  for (let person of data) {
      if (cmp_exist(person.name, '.mcard-name')) {
...
function make_quotes(data) {
  for (let person of data) {
      if (cmp_exist(person.name, '.quoute-title')) {
          let div = ` <div class="carousel-item">\
                          <div class="uno d-flex flex-md-row flex-column">\
                              ...
                              <div class="quote d-flex flex-column ml-md-5">\
                                <div class="quote-textc">\
                                  <p class="quoute-text">\
                                    « ${person.text}\
                                  </p>\
                                </div>\
                                <h3 class="quoute-title">${person.name}</h3>\
                                <p class="quoute-job">${person.title}</p>\
                              </div>\
                          </div>\
                      </div>`;
